function [cfg,group_cfg] = bramila_checkparameters_group(cfg,group_cfg)
    ...
    for j=1:length(cfg)
        if isfield(cfg{j},'volumes_to_use') && ~isempty(cfg{j}.volumes_to_use)
            frames(j)=max(cfg{j}.volumes_to_use);            
        else
    ...
    if length(unique(frames))>1
        ...
        warning('The number of volumes is not same for all subjects, skipping all ISC functionalities!')
    ...
    if length(unique(siz(:,1)))>1 || length(unique(siz(:,2)))>1 || length(unique(siz(:,3)))>1
        warning('Volume dimensions are not same for all subjects, skipping all ISC functionalities!')
        group_cfg.do_spatial_ISC=0;
        group_cfg.do_temporal_tissue_ISC=0;
    end
