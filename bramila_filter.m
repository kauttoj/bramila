...
elseif isfield(cfg,'infile') && ~isempty(cfg.infile)
    ...
    data=double(data);
    mdata=mean(data,4); % mean in time;
elseif isfield(cfg,'tsdata') && ~isempty(cfg.tsdata)
    data = cfg.tsdata;
else
...
fprintf('Filtering data (filter ''%s'')...',FILTERTYPE);
    ...
    switch FILTERTYPE
        ...
        case 'SPMhp'
            ...
            if length(siz)==2
                ...
                tsdata=reshape(tsdata,[],siz(4));
            end
            tsdata=double(transpose(tsdata));
            ...
            %tsdataout(:,maskID)=filtfilt(b,a,tsdata(:,maskID));
            tsdataout(:,maskID) = SPM_high_pass_filter(tsdata(:,maskID)',TR,1/HPF)';
            fprintf(' done\n');
            ...
            tsdataout=transpose(tsdataout);
            tsdataout=reshape(tsdataout,siz);
            cfg.vol = tsdataout;
            
            if WRITE==1 || nargout<1
                cfg.infile = bramila_savevolume(cfg,tsdataout,'masked, detrended, regressed and filtered EPI volume','mask_detrend_fullreg_filtered.nii');
                cfg.outfile = cfg.infile;
            end
        otherwise
            error('Something went wrong in filtering (not supposed to!)')
    end
end
