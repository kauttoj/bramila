    ...
    if (~isfield(tempcfg{subj},'plot') || (isfield(tempcfg{subj},'plot') && tempcfg{subj}.plot == 1) )
        ...
        if(~isfield(tempcfg{subj},'mask'))
            ...
            if(all(size(tempcfg{subj}.mask)==[91 109 91]))
                ...
                atlasmasknii=load_nii([tempcfg{subj}.bramilapath,filesep,'external/HO/mask.nii']); %1==WM, 2==CSF, 3==GM, 4==SUB, 5==CEREBELL
                atlasmask=atlasmasknii.img(:,:,:,1).*tempcfg{subj}.mask;
                
                if isfield(tempcfg{subj},'vol') && ~isempty(tempcfg{subj}.vol)
                    
                elseif isfield(tempcfg{subj},'infile') && ~isempty(tempcfg{subj}.infile)
                    nii=load_nii(tempcfg{subj}.infile);
                    tempcfg{subj}.vol=nii.img;
                    clear nii;
                else
                    error('No input EPI data found!');
                end
                
                vol=single(tempcfg{subj}.vol);
                T=size(vol,4);
                data=reshape(vol,[],T);
                ...
                daspect(dtemp);
                saveas(gcf,[tempcfg{subj}.outpath '/bramila/diagnostics_time.eps'],'psc2');
                figure
                ...
                saveas(gcf,[tempcfg{subj}.outpath '/bramila/diagnostics_corrMat.eps'],'psc2');
                save([tempcfg{subj}.outpath '/bramila/diagnostics.mat'],'gmIDs','wmIDs','csfIDs', 'gsIDs','FD', 'DV', 'SD', 'GS', 'GM', 'WM', 'CSF');
            end
        end
    end    
    tempcfg{subj}.vol=[];    
